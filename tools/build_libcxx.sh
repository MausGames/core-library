#!/bin/bash

_PATH_="$(dirname "$(realpath "$0")")"

_PREFIX_="$_PATH_/../output/libcxx"
_GLIBC_="$_PATH_/../output/glibc"

_CMAKE_FLAGS_="-DCMAKE_INSTALL_PREFIX=$_PREFIX_ -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_BUILD_TYPE=MinSizeRel -DCMAKE_INTERPROCEDURAL_OPTIMIZATION=TRUE -DCMAKE_PLATFORM_NO_VERSIONED_SONAME=TRUE"

export CC="clang"
export CXX="clang++"
export LD="mold"
export CFLAGS="-Os -flto -fno-semantic-interposition -fvisibility=hidden -msse3 -isystem $_GLIBC_/include"
export CXXFLAGS="-Os -flto -fwhole-program-vtables -fno-semantic-interposition -fvisibility=hidden -msse3 -isystem $_GLIBC_/include"
export LDFLAGS="-fuse-ld=mold -flto -L$_GLIBC_/lib -Wl,-rpath,\$ORIGIN -Wl,--gc-sections -Wl,--icf=all -Wl,--ignore-data-address-equality -Wl,--allow-shlib-undefined -z nodlopen"

cd "$_PATH_/../modules/llvm-project"
rm -r build
mkdir build
cmake $_CMAKE_FLAGS_ -G Ninja -S runtimes -B build -DLLVM_ENABLE_RUNTIMES="libcxx;libcxxabi" -DLIBCXX_ENABLE_STATIC=OFF -DLIBCXX_ENABLE_EXCEPTIONS=OFF -DLIBCXX_ENABLE_RTTI=OFF -DLIBCXX_ENABLE_ASSERTIONS=OFF -DLIBCXX_INCLUDE_TESTS=OFF -DLIBCXX_INCLUDE_BENCHMARKS=OFF -DLIBCXX_ABI_UNSTABLE=ON -DLIBCXX_ENABLE_STATIC_ABI_LIBRARY=ON -DLIBCXX_ENABLE_ABI_LINKER_SCRIPT=OFF
ninja -C build cxx cxxabi
ninja -C build check-cxx check-cxxabi
ninja -C build install-cxx install-cxxabi