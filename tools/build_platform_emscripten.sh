...
_CMAKE_FLAGS_="-S .. -B . -DCMAKE_PREFIX_PATH=$_PREFIX_ -DCMAKE_INSTALL_PREFIX=$_PREFIX_ -DCMAKE_BUILD_TYPE=MinSizeRel -DCMAKE_INTERPROCEDURAL_OPTIMIZATION=TRUE -DCMAKE_POLICY_DEFAULT_CMP0069=NEW -DCMAKE_TOOLCHAIN_FILE=$_EMSCRIPTEN_/cmake/Modules/Platform/Emscripten.cmake"
...
cd build
rm CMakeCache.txt
cmake $_CMAKE_FLAGS_ -DBUILD_SHARED_LIBS=OFF -DSDL_AUDIO=OFF -DSDL_RENDER=OFF -DSDL_HAPTIC=OFF -DSDL_OFFSCREEN=OFF -DSDL_VULKAN=OFF -DSDL_METAL=OFF -DSDL_WAYLAND_QT_TOUCH=OFF -DSDL_DBUS=OFF -DSDL_VIRTUAL_JOYSTICK=OFF -DSDL_RPATH=OFF -DSDL_TEST=OFF
...
cd build
rm CMakeCache.txt
cmake $_CMAKE_FLAGS_ -DBUILD_SHARED_LIBS=OFF -DSDL2TTF_HARFBUZZ=ON -DSDL2TTF_VENDORED=ON -DSDL2TTF_SAMPLES=OFF
cmake --build . --parallel
cmake --install .
